...
// SimpleAgent wraps an llm.Client and calls Generate once; the response text is
// returned as Result.Answer with no tool calls or multi-step reasoning.
//
// In production, replace newScriptedLLM with a real llm.Client (e.g. OpenAI-compatible).
func ExampleSimpleAgent() {
	// Use a deterministic scripted LLM for testable output.
	// In production: plug your own llm.Client implementation.
