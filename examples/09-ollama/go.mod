module github.com/costa92/llm-agent/examples/09-ollama

go 1.26.0

require (
	github.com/costa92/llm-agent v0.2.0
	github.com/costa92/llm-agent-contract v0.2.0
	github.com/costa92/llm-agent-providers v0.3.0
)

require (
	github.com/anthropics/anthropic-sdk-go v1.41.0 // indirect
	github.com/bahlo/generic-list-go v0.2.0 // indirect
	github.com/buger/jsonparser v1.1.2 // indirect
	github.com/google/uuid v1.6.0 // indirect
	github.com/invopop/jsonschema v0.13.0 // indirect
	github.com/mailru/easyjson v0.7.7 // indirect
	github.com/ollama/ollama v0.23.2 // indirect
	github.com/openai/openai-go/v3 v3.35.0 // indirect
	github.com/standard-webhooks/standard-webhooks/libraries v0.0.0-20260427160145-3afa6683f8b2 // indirect
	github.com/tidwall/gjson v1.18.0 // indirect
	github.com/tidwall/match v1.1.1 // indirect
	github.com/tidwall/pretty v1.2.1 // indirect
	github.com/tidwall/sjson v1.2.5 // indirect
	github.com/wk8/go-ordered-map/v2 v2.1.8 // indirect
	golang.org/x/crypto v0.49.0 // indirect
	golang.org/x/sync v0.20.0 // indirect
	golang.org/x/sys v0.42.0 // indirect
	gopkg.in/yaml.v3 v3.0.1 // indirect
)

// This demo lives in its own module so the heavy provider dependency tree
// (Ollama / OpenAI / Anthropic SDKs) stays isolated here and never leaks
// into the offline-by-default `github.com/costa92/llm-agent/examples`
// module. Local-dev replaces point at the sibling repos in the umbrella;
// the release tooling pins them to tags.
replace github.com/costa92/llm-agent => ../../

replace github.com/costa92/llm-agent-providers => ../../../llm-agent-providers
