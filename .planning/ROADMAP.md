...
**Plans (final 7-plan layout, after planner expanded with the Wave-0 typed-errors prerequisite):** `01-01` typed errors in core (`llm/errors.go` extension + tag `v0.3.0-pre.2`) — Wave 0, blocks all adapter work. `01-02` (OpenAI), `01-03` (Anthropic), `01-04` (Ollama) — Wave 1, parallel, target unrelated sister-repo subpackages. `01-05` (`internal/contract` harness + capture scripts + goleak) — Wave 2, depends on all 3 adapters. `01-06` (`nightly-ollama-live.yml`) and `01-07` (Provider Author Guide v0.1) — Wave 3, parallel, both depend on `01-05`.
