...
1. **Q1: Should `llm.Tool`, `llm.Message`, `llm.FinishReason` remain shared between LegacyClient and new ChatModel, or should they be duplicated under different names?**
   - **RESOLVED:** Share the types between LegacyClient and ChatModel surfaces; document the deliberate sharing in `legacy.go`. `ToolCall` adds an optional `ID` field (back-compat — v0.2 callers don't construct it).
...
2. **Q2: Where does the `go doc ./...` baseline snapshot for Pitfall 22 live, and is its update automated?**
   - **RESOLVED:** Single rolling file `docs/api-snapshot.txt` regenerated via `go doc ./... > docs/api-snapshot.txt` at every phase exit. Phase 0 captures the first snapshot as the final task of plan 00-05; automated drift detection deferred to Phase 7.
...
3. **Q3: When exactly is `v0.3.0-pre.1` tagged?**
   - **RESOLVED:** Tag `v0.3.0-pre.1` as the LAST step of Phase 0, after all 4 repos' Phase 0 commits land. Until then, sister-repo `require` lines reference `v0.3.0-pre.1` as a placeholder (umbrella CI overrides via `go.work`); first-CI on sister repos may be RED until tagging — that's expected and documented.
...
4. **Q4: Does the umbrella CI need to handle the case where one of the sister repos' main branch doesn't exist yet?**
   - **RESOLVED:** Plan ordering enforces sister repos exist BEFORE umbrella.yml lands — `00-03 (sister skeletons)` is Wave 1; `00-05 (umbrella CI)` is Wave 3 with explicit `depends_on: [03]`. The dependency graph closes the gap window.
...
5. **Q5: Should `Embedder` be in a different file than `ToolCaller` and `StructuredOutputs` since it doesn't embed `ChatModel`?**
   - **RESOLVED:** Keep all three capability interfaces in `capabilities.go`. The fact that `Embedder` does not embed `ChatModel` is locally visible from the type signature; godoc renders them grouped. No separate `embedder.go` file.
...
6. **Q6: Should the `release-precheck.yml` workflow file be a single shared file (reusable workflow) or 4 copies?**
   - **RESOLVED:** Copy-paste — 4 byte-identical copies of `release-precheck.yml`, one per repo. Reusable workflows would introduce sister-repo → core-repo workflow coupling, which is exactly the kind of implicit dependency Phase 0's multi-repo discipline rejects. Revisit in Phase 7 if the duplication becomes painful.
